...
st.markdown(
    "<h1 style='text-align: center; font-size: 38px; color: #5f1f9c ; font-weight: 700;font-family:PhonePeSans;'>Phonepe Pulse Data Visualization and Exploration </h1>",
    unsafe_allow_html=True,
)
...
tbls = fetch_from_sql_to_df(
    "phonepe_pulse", get_db_connection()[1], get_db_connection()[0]
)

# st.write(tbls)
# st.write(get_db_connection()[1])
...
if selected == "Datasets":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:rgb(235, 48, 84) ; font-weight: 700;font-family:PhonePeSans;'>Datasets and it's insights </h1>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    col.markdown(
        "<h3 style='text-align: left; color: rgb(235, 48, 84);font-family:PhonePeSans;'> Select Dataset to Explore and Download: </h3>",
        unsafe_allow_html=True,
    )
    option = col.selectbox(label="", options=list(tbls.keys()), key="df")

    st.markdown(
        "<style>div.css-1jpvgo6 {font-size: 16px; font-weight: bolder;font-family:PhonePeSans; } </style>",
        unsafe_allow_html=True,
    )

    try:
        with get_db_connection()[1] as connection:
            st.write("")
    ...
    with tab1:
        column1, column2, buffer = st.columns([2, 2, 1])

        st.markdown(
            """ <style> div.stButton > button:first-child {background-color: red    ;color:white;font-size:8px;height:3em;width:30em;border-radius:5px 5px 5px 5px;}  </style>""",
            unsafe_allow_html=True,
        )
        ...
        if show_df:
            ...
            st.data_editor(
                data=tbls[option].head(100), use_container_width=True, height=1000
            )
...
def filter_list(df, suff, multi_st=False):
    ...
    if multi_st:
        state_opts = col1.multiselect(
            f"State (Select Multiple)", state_list, key=f"state_opts{suff}"
        )
    else:
        state_opts = col1.selectbox(
            f"State", ["All"] + list(map(str, state_list)), key=f"state_opts{suff}"
        )
    ...
    **kwargs,
    ...
    if chart_type == "lne":
        ...
        if "text_auto" in kwargs:
            ...
            fig = px.bar(
                data, x=x_column, y=y_column, text=y_column, **kwargs
            ).update_layout(width=widthv, height=heightv)
        else:
            fig = px.bar(data, x=x_column, y=y_column, **kwargs).update_layout(
...
def filter_top_trans_dist(top_trans_dist, year, quarter):
    ...
    if quarter != "All":
        filtered_top_trans_dist = filtered_top_trans_dist[
            filtered_top_trans_dist["Quarter"] == quarter
        ]
...
if selected == "Overview":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:#dd9933 ; font-weight: 700;font-family:PhonePeSans;'>Overview </h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<h3 style='text-align: left; color: #dd9933;font-family:PhonePeSans;'> Comprehensive Highlights of PhonePe Pulse Data: </h3>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    style_metric_cards(
        background_color="black", border_color="Teal", border_left_color="Teal"
    )
    ...
    col3.metric(
        ...
        value="{:.2f} Cr".format(agg_trans / 10000000),
        delta="Forward Trend",
    )
    ...
    col4.metric(
        ...
        value="{:.2f} K Cr.".format(agg_trans / 10000000000),
        delta="Forward Trend",
    )
    add_vertical_space(1)

    Tran_type = (
        tbls["agg_tran"].groupby(["Transaction_type"])["Transaction_count"].sum()
    )
    ...
    st.markdown(
        "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Key Primary Observations  </h3>",
        unsafe_allow_html=True,
    )

    st.markdown(
        "<h3 style='text-align:left; font-size: 20px;color: #dd9933;font-family:PhonePeSans;'>Let's explore some fundamental findings from the Phonepe Pulse data.  </h3>",
        unsafe_allow_html=True,
    )
    ...
    with Dir:
        if ques == Ques_1:
            agg_trans_data = (
                tbls["agg_tran"]
                .groupby("Region")
                .agg({"Transaction_amount": "sum", "Transaction_count": "sum"})
                .reset_index()
            )
            agg_trans_data["Transaction_count"] = round(
                agg_trans_data["Transaction_count"] / 1e10, 2
            )
            agg_trans_data["Transaction_amount"] = round(
                agg_trans_data["Transaction_amount"] / 1e10, 2
            )

            agg_trans_data1 = (
                tbls["agg_tran"]
                .groupby("Year")["Transaction_count"]
                .sum()
                .reset_index()
            )
            agg_trans_data1["Transaction_count"] = round(
                agg_trans_data1["Transaction_count"] / 10000000, 2
            )

            st.markdown(
                "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Transaction Count and Amount by Region  </h3>",
                unsafe_allow_html=True,
            )
            st.info(
                "The Southern region holds a prominent position over all other regions in terms of Transaction Count."
            )
            agg_col1, agg_col2, agg_col3 = st.columns([3, 0.1, 3])
            qry_df = SQL_QRY(
                "SELECT Region, SUM(Transaction_amount) AS Amount, SUM(Transaction_count) AS Count FROM agg_tran GROUP BY Region;"
            )
            st.write(qry_df)
            agg_col1.plotly_chart(
                ...
                .update_traces(
                    ...
                    customdata=qry_df["Amount"],
                    hovertemplate="<b>%{label}</b><br>Transaction Count: %{value:,.2f} Billion  </br> Transaction Amount: ₹ %{customdata:.2f} Billion",
            ...
            agg_col2.markdown(
                "<div style='border-left: 3px solid #5f1f9c;font-family:PhonePeSans; height: 380px;'></div>",
                unsafe_allow_html=True,
            )
            ...
            agg_col3.plotly_chart(
                ...
                .update_traces(
                    ...
                    hoverinfo="label+percent+value",
                    customdata=qry_df["Count"],
                    hovertemplate="<b>%{label}</b><br>Transaction Amount: %{value:,.2f} Billion. </br> Transaction Count: ₹ %{customdata:.2f} Billion",
                    hoverlabel=dict(font_size=14, font_family="Arial"),
                )
                .update_layout(
                    width=400,
            ...
            expand_data = agg_trans_data.loc[
                :, ["Region", "Transaction_count", "Transaction_amount"]
        ...
        if ques == Ques_2:
            st.markdown(
                "<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True
            )
            type_col1, type_col2, type_col3 = st.columns([2.5, 0.01, 3])

            agg_trans_data2 = (
                tbls["agg_tran"]
                .groupby(["Transaction_type", "Year"])["Transaction_count"]
                .sum()
                .reset_index()
            )
            agg_trans_data2["Transaction_count"] = round(
                agg_trans_data2["Transaction_count"] / 10000000, 2
            )
            agg_trans_data3 = (
                tbls["agg_tran"]
            ...
            agg_trans_data3["Transaction_count"] = round(
                agg_trans_data3["Transaction_count"] / 10000000, 2
            )

            type_col1.markdown(
                "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Transaction Count in Cr. by  type  </h3>",
                unsafe_allow_html=True,
            )
            type_col1.info(
                "The Merchant  payment holds a prominent position over all transaction type."
            )
            ...
            type_col2.markdown(
                "<div style='border-left: 3px solid #5f1f9c;font-family:PhonePeSans; height: 550px;'></div>",
                unsafe_allow_html=True,
            )

            type_col3.markdown(
                "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Transaction  Count by Year and Type </h3>",
                unsafe_allow_html=True,
            )
            type_col3.info(
                "Yearly, overall transactions rise; Peer to Peer and Merchants notably grow by over 200%."
            )
        ...
        if ques == Ques_3:
            ...
            agg_user = (
                tbls["agg_user"]
                .groupby("Brand")["Transaction_count"]
                .sum()
                .reset_index()
            )
            t_hold = 12388624
                ...
                agg_user = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            {
                                "Brand": ["Others"],
                                "Transaction_count": [small_brands_sum],
                            }
                        ),
                    ],
                    ignore_index=True,
            ...
            agg_user1 = (
                tbls["agg_user"]
                .groupby(["Year", "Brand"])["Transaction_count"]
                .sum()
                .reset_index()
            )
            ...
            if "Others" not in agg_user1["Brand"].values:
                ...
                agg_user1 = agg_user1.sort_values(
                    by=["Year", "Transaction_count"], ascending=[True, True]
                )

            others = pd.DataFrame(
                {
                    "Brand": ["Others"],
                    "Transaction_count": [small_brands["Transaction_count"].sum()],
                }
            )
            agg_user1 = agg_user1[agg_user1["Transaction_count"] >= t_hold]
            agg_user1 = pd.concat([agg_user1, others], ignore_index=True)
            agg_user1["Transaction_count"] = round(
                agg_user1["Transaction_count"] / 1000000, 2
            )
            agg_users1 = agg_user1.sort_values(by="Year", ascending=True)

            agg_user["Transaction_count"] = round(
                agg_user["Transaction_count"] / 1000000, 2
            )
            user_col1.markdown(
                "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Transaction Count in Cr. by  Brand  </h3>",
                unsafe_allow_html=True,
            )
            user_col1.info(
                "Xiaomi phones dominate, suggesting PhonePe's popularity extends to economically diverse users."
            )
            ...
            user_col3.markdown(
                "<h3 style='text-align:center; font-size: 30px;color: #dd9933;font-family:PhonePeSans;'>Transaction  Count in Cr. by Brand, Year  </h3>",
                unsafe_allow_html=True,
            )
            user_col3.info(
                "The transaction count increases year by year, regardless of the device used."
            )
        ...
        if ques == Ques_4:
            ...
            expander4.write(
                df.loc[
                    :, ["State", "Year", "Quarter", "District", "Transaction_amount"]
                ].reset_index(drop=True)
            )
        ...
        if ques == Ques_6:
            ...
            t_map.update_traces(
                ...
                hovertemplate="<b>%{label}</b><br>Transaction Count: %{customdata[0]:,.2f} lakhs<br>Percentage: %{customdata[1]:.2%}",
            )

            st.plotly_chart(
                t_map.update_traces(hoverlabel=dict(font_size=14, font_family="Arial")),
                use_container_width=True,
            )

            # st.write(df.columns)
            expander3 = st.expander(
                label="Transaction Count  Percentage by Brand, Detailed view"
            )
            expander3.write(
                df.loc[
                    :,
        ...
        if ques == Ques_7:
            ...
            if year_opts == "2018":
                ...
                reg_user_plot = px.density_mapbox(
                    df,
                    lat="Latitude",
                    lon="Longitude",
                    z="App_opens",
                ...
                reg_user_plot.update_traces(
                    hovertemplate="<b>%{hovertext}</b><br>App Opens: %{z:,.2f} Lakhs<br>Registered Users: %{customdata[3]:,.2f} Lakhs<br>Region - %{customdata[4]}"
                )
                reg_user_plot.add_annotation(
            ...
            # st.write(df.info())
            st.expander1 = st.expander(
                label="App opens Hotspots Detailed view", expanded=expanded
            )
            st.expander1.write(
                df.loc[
                    :, ["State", "District", "Year", "App_opens", "Registered_users"]
                ].reset_index(drop=True)
            )
        ...
        if ques == Ques_8:
            ...
            trans_df1["Transaction_amt"] = round(
                trans_df1["Transaction_amount"] / 1e12, 2
            )
            ab = create_plotly_charts(
                trans_df1,
                "Treemap",
                ["Region", "Year"],
                "Transaction_amt",
            ...
            ab.update_layout(
                ...
                marker=dict(cornerradius=11),
                root_color="red",
                hovertemplate="<b>%{label}</b><br>Transaction Amount: %{customdata[0]:,.2f} Trillions.",
            )
        ...
        if ques == Ques_9:
            ...
            reg_user_plot = px.scatter_mapbox(
                df,
                lat="Latitude",
                lon="Longitude",
        ...
        if ques == Ques_10:
            ...
            filtered_top_pin = (
                pin_tran.groupby("Pincode")["Transaction_amount"]
                .sum()
                .reset_index()
            ...
            filtered_top_pin["Transaction_amount"] = (
                filtered_top_pin["Transaction_amount"] / 10000000
            )
            ...
            st.plotly_chart(
                ...
                .update_traces(
                    texttemplate="₹%{x:,.2f} Cr",
                    hovertemplate="<br>Transaction Amount: ₹%{x:,.2f} Cr",
                )
                .update_layout(
                    yaxis_type="category",
                    xaxis={"categoryorder": "total descending"},
    ...
    with SQL:
        if ques == Ques_1:
            qry_df2 = SQL_QRY(
                'SELECT "Region", SUM("Transaction_count") as Transactions, SUM("Transaction_amount") AS Amount FROM agg_tran GROUP BY "Region";'
            )

            # qry_df2['Tran in Cr.']=qry_df2['Transactions'].astype(float)
            qry_df2["Transactions"] = round(
                qry_df2["Transactions"].astype(float) / 1e10, 2
            )
            ...
            agg_col3.plotly_chart(
                ...
                .update_traces(
                    ...
                    textfont=dict(size=15, color="#FFFFFF"),
                    hoverinfo="label+percent+value",
                    customdata=qry_df2["Transactions"],
                    hovertemplate="<b>%{label}</b><br>transaction Amount: %{value:,.2f} Billion. </br> transaction Count: ₹ %{customdata:.2f} Billion",
                )
                .update_layout(
                    width=400,
        ...
        if ques == Ques_2:
            qry_df3 = SQL_QRY(
                'SELECT  "Transaction_type", SUM("Transaction_count") as Transactions  FROM agg_tran GROUP BY "Transaction_type";'
            )
            qry_df4 = SQL_QRY(
                'SELECT "Year", "Transaction_type", SUM("Transaction_amount") AS Amount FROM agg_tran GROUP BY "Year","Transaction_type";'
            )
            ...
            qry_df3["Trans_Cr."] = round(
                qry_df3["Transactions"].astype(float) / 10000000, 2
            )
            ...
            type_col2.markdown(
                "<div style='border-left: 3px solid #5f1f9c;font-family:PhonePeSans; height: 630px;'></div>",
                unsafe_allow_html=True,
            )
        ...
        if ques == Ques_3:
            ...
            qry_df = SQL_QRY(
                'SELECT "Brand", SUM("Transaction_count") AS Transactions FROM agg_user GROUP BY "Brand";'
            )
            ...
            qry_df1 = SQL_QRY(
                'SELECT "Region", "Brand", SUM("Transaction_count") AS Transaction_count FROM agg_user GROUP BY "Region" , "Brand";'
            )

            qry_df1["Count in Cr."] = round(
                qry_df1["Transaction_count"].astype(float) / 10000000, 2
            )
        ...
        if ques == Ques_4:
            qry_df5 = SQL_QRY(
                "SELECT Year, Quarter, State, SUM(Transaction_count) AS Transactions,SUM(Transaction_amount) AS Amount FROM top_tran_dist GROUP BY Year, Quarter, State;"
            )
            ...
            expander4 = st.expander(label="Top Transactions by State, Detailed view")
            expander4.write(
                df.loc[:, ["State", "Year", "Quarter", "Amount"]].reset_index(drop=True)
            )

        if ques == Ques_5:
            qry_df6 = SQL_QRY(
                "SELECT Year, Quarter, State, District, SUM(Transaction_count) AS Transactions,SUM(Transaction_amount) AS Amount FROM top_tran_dist GROUP BY Year, Quarter, State,District;"
            )
            ...
            expander44.write(
                df.loc[
                    :, ["State", "District", "Year", "Quarter", "Amount"]
                ].reset_index(drop=True)
            )

        if ques == Ques_6:
            qry_df7 = SQL_QRY(
                'SELECT Year, Quarter, Brand, State, SUM(Transaction_count) AS Transaction_count,sum(Percentage) as Percentage FROM agg_user GROUP BY Year, Quarter, "Brand", State;'
            )
        ...
        if ques == Ques_7:
            qry_df8 = SQL_QRY(
                "SELECT Year, Quarter, Region,State, District, SUM(Registered_users) AS Registered_users,SUM(App_opens) AS App_opens,Latitude,Longitude FROM map_user GROUP BY Year, Quarter, Region,State,District,Latitude,Longitude;"
            )
        ...
        if ques == Ques_8:
            ...
            qry_df8 = SQL_QRY(
                'SELECT "Year","Region", "State",  SUM("Transaction_amount") AS Transaction_amount FROM agg_tran GROUP BY "Year", "Region", "State";'
            )
            ...
            qry_df8 = (
                qry_df8.groupby("Region")
                .agg({"Transaction_amount": "sum"})
                .reset_index()
            )
            st.expander11 = st.expander(
                label="Regionwise Transaction amount", expanded=False
            )
            st.expander11.write(
                qry_df8.loc[:, ["Region", "Transaction_amount"]].reset_index(drop=True)
            )
        ...
        if ques == Ques_9:
            qry_df9 = SQL_QRY(
                "SELECT Year,Quarter,Region, Latitude,Longitude,State,  District, SUM(Registered_users) AS Registered_users,SUM(App_opens) AS App_opens FROM map_user GROUP BY Year,Quarter,Region, Latitude,Longitude,State,  District;"
            )

            df, qtr_opts, state_opts, year_opts = filter_list(qry_df9, suff=91)
            df["Year"] = (
                df["Year"].astype(str).str.strip().str.replace(",", "").astype(int)
            )
        ...
        if ques == Ques_10:
            qry_df10 = SQL_QRY(
                'SELECT "Pincode", SUM("Transaction_amount") AS Transaction_amount FROM top_tran_pin GROUP BY "Pincode";'
            )

            pin_tran = tbls["top_tran_pin"]
            qry_df10_fil = (
                qry_df10.groupby("Pincode")["Transaction_amount"]
                .sum()
                .reset_index()
                .sort_values("Transaction_amount", ascending=False)
                .head(10)
            ...
            qry_df10_fil["Transaction_amount"] = (
                qry_df10_fil["Transaction_amount"] / 10000000
            )
            title17 = f"Top 10 pincode locations by Transaction volume. Value in Crores"
            st.plotly_chart(
                create_plotly_charts(
...
if selected == "Transactions":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:#60b4ff ; font-weight: 700;font-family:PhonePeSans;'>Transactions Overview </h1>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    chart_title = (
        f"Transaction details of {state_opts} for  {qtr_opts}{suffix} of {year_opts}."
    )
    df = (
        df.groupby(["State", "Transaction_type", "Year", "Quarter"])[
            "Transaction_amount"
        ]
        .sum()
        .reset_index()
    ...
    st.plotly_chart(
        ...
        .update_yaxes(
            tickformat=",.2f ",
            title="Amount in thousand Crores",
            ticksuffix="  Cr",
            tickprefix="₹",
            title_font=dict(size=18),
            tickfont=dict(size=15),
    ...
    expander1.write(
        sorted_df.loc[
            :, ["Quarter", "Transaction_type", "Transaction_amount"]
        ].reset_index(drop=True)
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    map_user_plot = px.scatter_mapbox(
        df,
        lat="Latitude",
        lon="Longitude",
    ...
    map_user_plot.update_traces(
        hovertemplate="<b>%{text}</b><br>Transaction Count: ₹%{customdata[2]:,.2f} lakhs<br>Transaction Amount: ₹%{customdata[3]:,.2f} Cr<br>Quarter: %{customdata[4]}"
    )
    ...
    buff.markdown(
        "<div style='border-left: 3px solid #5f1f9c;font-family:PhonePeSans; height: 400px;'></div>",
        unsafe_allow_html=True,
    )
...
if selected == "Users":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:#ff4b4b ; font-weight: 700;font-family:PhonePeSans;'>Users Overview </h1>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    top_dist_user_grpy_df = (
        df.groupby("District")["Registered_users"].sum().nlargest(10).index.tolist()
    )
    ...
    st.plotly_chart(
        ...
        .update_traces(
            hovertemplate="<b>%{y}</b><br>Registered Users: %{x:,.2f} Lakhs<br>State: %{customdata[0]}"
        ),
        use_container_width=True,
    )
...
if selected == "Trend":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:#3dd56d ; font-weight: 700;font-family:PhonePeSans;'>Trend Analysis </h1>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    year = col4.selectbox(
        "Year", ["All"] + list(map(str, map_tran["Year"].unique())), key="Year "
    )
    ...
    tab1.plotly_chart(
        ...
        .update_layout(
            height=500,
            width=900,
            yaxis_title="Count in Crores",
            title={"x": 0.5, "xanchor": "center", "y": 0.9, "yanchor": "bottom"},
        )
        .update_traces(
            textposition="bottom right",
            texttemplate="%{y:,.2f} K",
            hovertemplate="<b>%{customdata[1]}</b><br>Transaction Count: %{y:,.2f} K<br>Transaction Amount: ₹%{customdata[0]:,.2f} Cr",
        )
    ...
    expander6.write(
        (
            df.loc[
                :, ["Region", "District", "Year", "Quarter", "Transaction_count"]
            ].reset_index(drop=True)
        )
    )
    ...
    tab2.plotly_chart(
        ...
        .update_traces(
            textposition="bottom right",
            texttemplate="%{y:,.2f} Cr",
            hovertemplate="<b> %{customdata[0]}</b><br>Transaction Amount: ₹%{y:,.2f} Cr<br>Transaction Count:%{customdata[1]:,.2f} K",
        )
    ...
    expander7.write(
        (
            df.loc[
                :, ["Region", "District", "Year", "Quarter", "Transaction_amount"]
            ].reset_index(drop=True)
        )
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    tab3, tab4 = st.tabs(
        ["📊Top District - Transaction Count", "💰Top District -  Transaction Amount"]
    )
    tab3.plotly_chart(
        create_plotly_charts(
            df2,
        ...
        .update_yaxes(title_text="", showgrid=False)
        .update_traces(
            texttemplate="%{x:,.2f} Lakhs",
            hovertemplate="<b>%{customdata[1]}</b><br>Transaction Count: %{x:,.2f} Lakhs<br>Transaction Amount: ₹%{customdata[0]:,.2f} Cr",
        )
        .update_layout(
            xaxis={"categoryorder": "total descending"},
            yaxis={"categoryorder": "total ascending"},
    ...
    expander9 = tab3.expander(
        label="Top Transactions count by Districts, Detailed view"
    )
    expander9.write(
        df2.loc[
            :, ["State", "Year", "Quarter", "District", "Transaction_count"]
    ...
    tab4.plotly_chart(
        ...
        .update_traces(
            texttemplate="₹%{x:,.2f} Cr",
            hovertemplate="<b>%{customdata[1]}</b><br>Transaction Amount: ₹%{x:,.2f} Cr<br>Transaction Count: %{customdata[0]:,.2f} Cr",
        )
        .update_layout(
            xaxis={"categoryorder": "total descending"},
            yaxis={"categoryorder": "total ascending"},
    ...
    expander10 = tab4.expander(
        label="Top Transactions amount by Districts, Detailed view"
    )
    expander10.write(
        df2.loc[
            :, ["State", "Year", "Quarter", "District", "Transaction_amount"]
    ...
    quarter3 = col7.selectbox(
        "Quarter", ["All"] + list(map(int, qtr_list)), key="quarter3"
    )
    ...
    filtered_top_states = (
        top_state.groupby("State")["Transaction_amount"]
        .sum()
        .reset_index()
        .sort_values("Transaction_amount", ascending=False)
        .head(10)
    ...
    filtered_top_dist = (
        top_dist.groupby("District")["Transaction_amount"]
        .sum()
        .reset_index()
        .sort_values("Transaction_amount", ascending=False)
        .head(10)
    ...
    filtered_top_pin = (
        top_pin.groupby("Pincode")["Transaction_amount"]
        .sum()
        .reset_index()
        .sort_values("Transaction_amount", ascending=False)
        .head(10)
    ...
    filtered_top_pin["Pincode"] = pd.Categorical(
        filtered_top_pin["Pincode"], ordered=True
    )
...
if selected == "Comparison":
    st.markdown(
        "<h1 style='text-align: center; font-size: 38px; color:white; font-weight: 700;font-family:PhonePeSans;'>Comparitive Analysis </h1>",
        unsafe_allow_html=True,
    )
    st.markdown("<hr style='border: 2px solid #5f1f9c;'>", unsafe_allow_html=True)
    ...
    st.markdown(
        "<h3 style='color: white;'>Regionwise Transaction amount comparison</h3>",
        unsafe_allow_html=True,
    )
    ...
    cat_fig = px.bar(
        ...
        facet_col="Region",
        facet_col_wrap=2,
        range_y=[0, trans_df1["Transaction_amount"].max() * 9],
    )

    cat_fig.update_yaxes(showgrid=False, tickformat="", tickprefix="₹")

    cat_fig.update_traces(
        hovertemplate="<b>%{text}</b><br>Transaction Amount: %{y} <br>Year : %{x:.0f} "
    )
    ...
    st.markdown(
        "<h3 style='color: white;'>Transaction count by Transaction type</h3>",
        unsafe_allow_html=True,
    )

    filtered_df, qtr_opts, state_opts, year_opts = filter_list(
        trans_df1, suff=10, multi_st=True
    )
    ...
    fig2 = px.bar(
        ...
        title=title10,
        labels=dict(
            Transaction_count="Transaction Count", Transaction_type="Transaction Type"
        ),
        hover_data={
            "State": True,
            "Transaction_count": True,
    ...
    if len(state_opts) != 0:
        fig2.update_traces(
            ...
            hovertemplate="<b>%{customdata[0]}</b><br>Transaction Count: %{y:,.2f} <br>Transaction Amount: ₹%{customdata[3]:,.2f}",
        )
    ...
    st.markdown(
        "<h3 style='color: white;'>Transaction amount  by Region, year, and  Quarterwise comparison</h3>",
        unsafe_allow_html=True,
    )
    ...
    fildf = trans_df2[
        (trans_df2["Region"] == reg_opts) & (trans_df2["Year"] == year_opt)
    ]

    fildf["Quarter"] = "Quarter " + fildf["Quarter"].astype(str)
    fildf["Transaction_amount"] = fildf["Transaction_amount"] / 1e9

    fig = create_plotly_charts(
        fildf,
        "Pie",
        "Quarter",
        "Transaction_amount",
        title=f"Transaction amount Comparison of {reg_opts} for the year {year_opt}",
        grid=False,
        hole=0.35,
        color_discrete_sequence=px.colors.sequential.Magma,
    )
